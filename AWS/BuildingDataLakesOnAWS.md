# Building Data Lakes on AWS course material links

_Please not that this is not a prep course for the "AWS Certified Data Engineer - Associate" (or any other) certification!_

This document contains links mentioned in the "Building Data Lakes on AWS" course. 

## Module 0: Course overview

Full name: Building Data Lakes on AWS

Product code: 200-ANBDLK-15

These links are based on the course version 1.5.3.

The official course material: [eVantage Bookshelf (VitalSource)](https://evantage.gilmoreglobal.com)

Additional material: [AWS Skill Builder](https://explore.skillbuilder.aws/)

## Module 1: Introduction to data lakes

### Websites

- [AWS serverless data analytics pipeline reference architecture](https://aws.amazon.com/blogs/big-data/aws-serverless-data-analytics-pipeline-reference-architecture)
- [Reference architecture](https://docs.aws.amazon.com/wellarchitected/latest/analytics-lens/reference-architecture.html)
- [AWS serverless data analytics pipeline reference architecture](https://aws.amazon.com/blogs/big-data/aws-serverless-data-analytics-pipeline-reference-architecture/)
- [Analytics on AWS](https://aws.amazon.com/big-data/datalakes-and-analytics/)

### Customer Stories

- [Yamato Logistics (HK)](https://aws.amazon.com/solutions/case-studies/yamato-logistics)
- [Siemens Mobility](https://aws.amazon.com/solutions/case-studies/siemens-mobility)
- [Bristol Myers Squibb](https://aws.amazon.com/solutions/case-studies/bristol-myers-squibb-case-study)
- [Netflix](https://aws.amazon.com/solutions/case-studies/netflix-kinesis-data-streams)
- [Epic Games](https://aws.amazon.com/solutions/case-studies/innovators/epic-games)

## Module 2: Data ingestion, cataloging, and preparation

### Websites

- [Data Lakes on AWS](https://aws.amazon.com/big-data/datalakes-and-analytics/datalakes/)
- [Zero-ETL Integrations](https://press.aboutamazon.com/2023/11/aws-announces-four-zero-etl-integrations-to-make-data-access-and-analysis-faster-and-easier-across-data-stores)
- [Using Amazon S3 Storage Lens to optimize your storage costs](https://docs.aws.amazon.com/AmazonS3/latest/userguide/storage-lens-optimize-storage.html)
- [Storage Best Practices for Data and Analytics Applications](https://docs.aws.amazon.com/whitepapers/latest/building-data-lakes/building-data-lake-aws.html)
- [Data ingestion methods](https://docs.aws.amazon.com/whitepapers/latest/building-data-lakes/data-ingestion-methods.html)
- [Autodesk Builds Unified Log Analytics Solution on AWS to Gain New Insights](https://aws.amazon.com/solutions/case-studies/autodesk-log-analytics/)
- [Sending data to a Firehose stream](https://docs.aws.amazon.com/firehose/latest/dev/basic-write.html)
- [Real-Time Insights on AWS Account Activity](https://aws.amazon.com/solutions/implementations/real-time-insights-account-activity/)
- [Catalog and search](https://docs.aws.amazon.com/whitepapers/latest/building-data-lakes/data-cataloging.html)
- [AWS Glue](https://aws.amazon.com/glue)
- [Built-in classifiers in AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/add-classifier.html#classifier-built-in)
- [Working with classifiers on the AWS Glue console](https://docs.aws.amazon.com/glue/latest/dg/console-classifiers.html)
- [Populating the AWS Glue Data Catalog](https://docs.aws.amazon.com/glue/latest/dg/populate-catalog-methods.html)
- [Data format options for inputs and outputs in AWS Glue for Spark](https://docs.aws.amazon.com/glue/latest/dg/aws-glue-programming-etl-format.html)
- [Data protection in AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/data-protection.html)
- [Amazon Athena](https://aws.amazon.com/athena)
- [Partitioning data in Athena](https://docs.aws.amazon.com/athena/latest/ug/partitions.html)
- [Top 10 Performance Tuning Tips for Amazon Athena](https://aws.amazon.com/blogs/big-data/top-10-performance-tuning-tips-for-amazon-athena)
- [Using Amazon Athena Federated Query](https://docs.aws.amazon.com/athena/latest/ug/connect-to-a-data-source.html)
- [Understanding columnar storage formats](https://docs.aws.amazon.com/athena/latest/ug/columnar-storage.html)

## Module 3: Building a data lake with AWS Lake Formation

### Websites

- [AWS Lake Formation](https://aws.amazon.com/lake-formation)
- [Getting started with Lake Formation](https://docs.aws.amazon.com/lake-formation/latest/dg/getting-started-setup.html#create-data-lake-admin)
- [Changing the default settings for your data lake](https://docs.aws.amazon.com/lake-formation/latest/dg/change-settings.html)
- [Upgrading AWS Glue data permissions to the AWS Lake Formation model](https://docs.aws.amazon.com/lake-formation/latest/dg/upgrade-glue-lake-formation.html)

## Module 4: Data processing and analytics

### Websites

- [AWS Glue announces AWS Glue Data Quality (Preview)](https://aws.amazon.com/about-aws/whats-new/2022/11/aws-glue-data-quality-preview)
- [Recipe step and function reference](https://docs.aws.amazon.com/databrew/latest/dg/recipe-actions-reference.html)
- [Redacting PII from application log output with Amazon Comprehend](https://aws.amazon.com/blogs/machine-learning/redacting-pii-from-application-log-output-with-amazon-comprehend)
- [AWS Glue PySpark transforms reference](https://docs.aws.amazon.com/glue/latest/dg/built-in-transforms.html)
- [Recipe step and function reference](https://docs.aws.amazon.com/databrew/latest/dg/recipe-actions-reference.html)
- [Editing AWS Glue managed data transform nodes](https://docs.aws.amazon.com/glue/latest/ug/edit-jobs-transforms.html)
- [AWS Marketplace / Masking/Tokenization](https://aws.amazon.com/marketplace/b/40013c0b-8380-4e29-b3ec-b8ae77f6fa45?category=40013c0b-8380-4e29-b3ec-b8ae77f6fa45)
- [Discover sensitive data by using custom data identifiers with Amazon Macie](https://aws.amazon.com/blogs/security/discover-sensitive-data-by-using-custom-data-identifiers-with-amazon-macie)
- [Use Macie to discover sensitive data as part of automated data pipelines](https://aws.amazon.com/blogs/security/use-macie-to-discover-sensitive-data-as-part-of-automated-data-pipelines)
- [Working with Spark jobs in AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/etl-jobs-section.html)
- [Streaming ETL jobs in AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/add-job-streaming.html)
- [Matching patient records with the AWS Lake Formation FindMatches transform](https://aws.amazon.com/blogs/big-data/matching-patient-records-with-the-aws-lake-formation-findmatches-transform)
- [Developing and testing AWS Glue job scripts locally](https://docs.aws.amazon.com/glue/latest/dg/aws-glue-programming-etl-libraries.html)
- [Working with Ray jobs in AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/author-job-ray.html)
- [Tutorial: Writing an AWS Glue for Spark script](https://docs.aws.amazon.com/glue/latest/dg/aws-glue-programming-intro-tutorial.html)
- [What is AWS Glue?](https://docs.aws.amazon.com/glue/latest/dg/console-jobs.html)
- [Getting started with AWS Glue interactive sessions](https://docs.aws.amazon.com/glue/latest/dg/interactive-sessions.html)
- [Resource-based policies within AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/glue-resource-policies.html)
- [Overview of workflows in AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/workflows_overview.html)
- [Running and monitoring a workflow in AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/running_monitoring_workflow.html)
- [Amazon CloudWatch](https://docs.aws.amazon.com/cloudwatch/)
- [Amazon EventBridge events](https://docs.aws.amazon.com/eventbridge/latest/userguide/eb-events.html)
- [What is Amazon CloudWatch Logs?](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs)
- [What Is AWS CloudTrail?](https://docs.aws.amazon.com/awscloudtrail/latest/userguide)

### Videos

- [Build Your Data Lake on Amazon S3 - AWS Online Tech Talks](https://youtu.be/ccIBYUjnt74)
- [Getting Started with AWS Glue ETL](https://www.youtube.com/watch?v=z3HeHlWg88M)
- [Build ETL Processes for Data Lakes with AWS Glue - AWS Online Tech Talks](https://www.youtube.com/watch?v=P3z4IX5-OHM)

### Courses

## Module 5: AWS Lake Formation Additional Configurations

### Websites

- [Blueprints and workflows in Lake Formation](https://docs.aws.amazon.com/lake-formation/latest/dg/workflows-about.html)
- [Creating a workflow](https://docs.aws.amazon.com/lake-formation/latest/dg/workflows-creating.html)
- [How Athena accesses data registered with Lake Formation](https://docs.aws.amazon.com/athena/latest/ug/lf-athena-access.html)
- [Managing Lake Formation permissions](https://docs.aws.amazon.com/lake-formation/latest/dg/managing-permissions.html)
- [Underlying data access control](https://docs.aws.amazon.com/lake-formation/latest/dg/access-control-underlying-data.html)
- [Metadata access control](https://docs.aws.amazon.com/lake-formation/latest/dg/access-control-metadata.html)
- [How Lake Formation application integration works](https://docs.aws.amazon.com/lake-formation/latest/dg/how-vending-works.html)
- [Lake Formation best practices, considerations, and limitations](https://docs.aws.amazon.com/lake-formation/latest/dg/lf-limitations.html)
- [Data filters in Lake Formation](https://docs.aws.amazon.com/lake-formation/latest/dg/data-filters-about.html)
- [Overview of data filtering](https://docs.aws.amazon.com/lake-formation/latest/dg/data-filtering-overview.html)
- [Lake Formation tag-based access control (TBAC)](https://docs.aws.amazon.com/lake-formation/latest/dg/tag-based-access-control.html)
- [Granting LF-Tag permissions using the console](https://docs.aws.amazon.com/lake-formation/latest/dg/TBAC-granting-tags-console.html)
- [Cross-account data sharing in Lake Formation](https://docs.aws.amazon.com/lake-formation/latest/dg/cross-account-permissions.html)
- [Cross-account data sharing best practices and considerations](https://docs.aws.amazon.com/lake-formation/latest/dg/cross-account-notes.html)
- [Lake Formation personas and IAM permissions reference](https://docs.aws.amazon.com/lake-formation/latest/dg/permissions-reference.html)
- [Amazon QuickSight](https://aws.amazon.com/Quicksight)
- [Managing SPICE memory capacity](https://docs.aws.amazon.com/quicksight/latest/user/managing-spice-capacity.html)
- [Publishing dashboards](https://docs.aws.amazon.com/quicksight/latest/user/creating-a-dashboard.html)
- [Working with embedded analytics](https://docs.aws.amazon.com/quicksight/latest/user/embedded-analytics.html)
- [Embed interactive dashboards in your application with Amazon QuickSight](https://aws.amazon.com/blogs/big-data/embed-interactive-dashboards-in-your-application-with-amazon-quicksight/)

### Videos

### Courses

- [Authoring Visual Analytics using Amazon QuickSight](https://www.aws.training/SessionSearch?courseId=107671&languageId=1)

## Module 6: Modern data architecture

### Websites

- [Analytics on AWS](https://aws.amazon.com/big-data/datalakes-and-analytics/)
- [Learn how to build Modern Data Analytics Solutions on AWS](https://aws.amazon.com/blogs/training-and-certification/learn-how-to-build-modern-data-analytics-solutions-on-aws)
- [Modern Data Analytics Reference Architecture on AWS](https://d1.awsstatic.com/architecture-diagrams/ArchitectureDiagrams/modern-data-analytics-using-lake-house-ra.pdf?anld_da6)
- [OpenSearch](https://opensearch.org)
- [Apache Spark on Amazon EMR](https://aws.amazon.com/emr/features/spark)
- [Apache Hive on Amazon EMR](https://aws.amazon.com/emr/features/hive)
- [Presto on Amazon EMR](https://aws.amazon.com/emr/features/presto)
- [Modern Data Reference Architecture](https://docs.aws.amazon.com/wellarchitected/latest/analytics-lens/reference-architecture.html)
- [Derive Insights from AWS Modern Data](https://docs.aws.amazon.com/whitepapers/latest/derive-insights-from-aws-modern-data/derive-insights-from-aws-modern-data.html)
- [Using Redshift Spectrum with AWS Lake Formation](https://docs.aws.amazon.com/redshift/latest/dg/spectrum-lake-formation.html)
- [Use an event-driven architecture to build a data mesh on AWS](https://aws.amazon.com/blogs/big-data/use-an-event-driven-architecture-to-build-a-data-mesh-on-aws)
- [Data Mesh Reference Architecture](https://docs.aws.amazon.com/wellarchitected/latest/analytics-lens/data-mesh-reference-architecture.html)
- [How JPMorgan Chase built a data mesh architecture to drive significant value to enhance their enterprise data platform](https://aws.amazon.com/blogs/big-data/how-jpmorgan-chase-built-a-data-mesh-architecture-to-drive-significant-value-to-enhance-their-enterprise-data-platform)
- [How GoDaddy built a data mesh to decentralize data ownership using AWS Lake Formation](https://aws.amazon.com/blogs/big-data/how-godaddy-built-a-data-mesh-to-decentralize-data-ownership-using-aws-lake-formation)
- [AWS Analytics Reference Architecture](https://github.com/aws-samples/aws-analytics-reference-architecture)
- [Build a Data Mesh on AWS](https://catalog.us-east-1.prod.workshops.aws/workshops/23e6326b-58ee-4ab0-9bc7-3c8d730eb851/en-US)

### Videos

### Courses

- [Building Batch Data Analytics Solutions on AWS](https://www.aws.training/SessionSearch?courseId=91270&languageId=1)
- [Building Data Analytics Solutions Using Amazon Redshift](https://www.aws.training/SessionSearch?courseId=80716&languageId=1)
- [Building Streaming Data Analytics Solutions on AWS](https://www.aws.training/SessionSearch?courseId=111638&languageId=1)
- [Authoring Visual Analytics using Amazon QuickSight](https://www.aws.training/SessionSearch?courseId=107671&languageId=1)

## Module 7: Course review and wrap-up

### Websites

- [Data Analytics Lens](https://docs.aws.amazon.com/wellarchitected/latest/analytics-lens/analytics-lens.html)
- [Modern Data Reference Architecture](https://docs.aws.amazon.com/wellarchitected/latest/analytics-lens/reference-architecture.html)
- [Configuration notes](https://docs.aws.amazon.com/wellarchitected/latest/analytics-lens/configuration-notes.html)
- [Derive Insights from AWS Modern Data](https://docs.aws.amazon.com/whitepapers/latest/derive-insights-from-aws-modern-data/derive-insights-from-aws-modern-data.html)
- [Monitoring and optimizing the data lake environment](https://docs.aws.amazon.com/whitepapers/latest/building-data-lakes/monitoring-optimizing-data-lake-environment.html)
- [AWS Glue Best Practices: Building a Performant and Cost Optimized Data Pipeline](https://docs.aws.amazon.com/whitepapers/latest/aws-glue-best-practices-build-performant-data-pipeline/aws-glue-best-practices-build-performant-data-pipeline.html)
- [AWS Pricing Calculator](https://calculator.aws/#/)
- [AWS Certification exam preparation](https://aws.amazon.com/certification/certification-prep/)
- [AWS Certification](https://aws.amazon.com/certification/)
- [AWS Recertification](https://aws.amazon.com/certification/recertification/)
- [All AWS Certification exams](https://aws.amazon.com/certification/exams/)
- [Schedule an Exam](https://aws.amazon.com/certification/certification-prep/testing/)
- AWS Skill Builder [subscription](https://aws.amazon.com/training/digital)
- [AWS Training and Certification](https://aws.amazon.com/training)
- [AWS Online Tech Talks](https://aws.amazon.com/events/online-tech-talks/on-demand/)
- [AWS Ramp-Up Guides](https://aws.amazon.com/training/ramp-up-guides/)

### Courses

- [AWS Skill Builder](https://explore.skillbuilder.aws/learn)
- [AWS Builder Labs](https://aws.amazon.com/training/digital/aws-builder-labs/)
- [AWS Workshops](https://workshops.aws/)
