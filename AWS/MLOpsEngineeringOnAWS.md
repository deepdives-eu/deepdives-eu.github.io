# MLOps Engineering on AWS course material links

_Please not that this is not a prep course for the "AWS Certified Machine Learning Engineer - Associate", "AWS Certified Machine Learning - Specialty" or any other certification!_

This document contains links mentioned in the "MLOps Engineering on AWS" course. 

## Module 0: Course Overview

Full name: MLOps Engineering on AWS

Product code: 200-MLPOPS-20

These links are based on the course version 2.0.4.

The official course material: [eVantage Bookshelf (VitalSource)](https://evantage.gilmoreglobal.com)

Additional material: [AWS Skill Builder](https://explore.skillbuilder.aws/)

## Module 1: Introduction to MLOps

### Websites

- [MLOps Principles](https://ml-ops.org/content/mlops-principles)
- [Hidden Technical Debt in Machine Learning Systems](https://papers.nips.cc/paper/2015/file/86df7dcfd896fcaf2674f757a2463eba-Paper.pdf)
- [Amazon SageMaker Studio Classic](https://docs.aws.amazon.com/sagemaker/latest/dg/studio.html)
- [Secure Data Science With Amazon SageMaker Studio Reference Architecture](https://github.com/aws-samples/amazon-sagemaker-studio-secure-data-science-workshop#private-network-per-amazon-sagemaker-studio-and-data-science-environment)
- [Amazon EMR](http://aws.amazon.com/emr)
- [Amazon Athena](http://aws.amazon.com/athena)
- [AWS Glue](https://aws.amazon.com/glue)
- [AWS CodeCommit](https://aws.amazon.com/codecommit/)

## Module 2: Initial MLOps /Experimentation Environments in SageMaker Studio

### Websites

- [Use Amazon SageMaker Studio Classic Notebooks](https://docs.aws.amazon.com/sagemaker/latest/dg/notebooks.html)
- [How Are Amazon SageMaker Studio Classic Notebooks Different from Notebook Instances?](https://docs.aws.amazon.com/sagemaker/latest/dg/notebooks-comparison.html)
- [Use Docker containers to train and deploy models](https://docs.aws.amazon.com/sagemaker/latest/dg/docker-containers.html)
- [Use Amazon SageMaker built-in algorithms or pretrained models](https://docs.aws.amazon.com/sagemaker/latest/dg/algos.html)
- [Use Pre-built SageMaker Docker images](https://docs.aws.amazon.com/sagemaker/latest/dg/docker-containers-prebuilt.html)
- [Machine Learning Frameworks and Languages](https://docs.aws.amazon.com/sagemaker/latest/dg/frameworks.html)
- [Available Deep Learning Containers Images](https://github.com/aws/deep-learning-containers/blob/master/available_images.md)
- [What are AWS Deep Learning Containers?](https://docs.aws.amazon.com/deep-learning-containers/latest/devguide/index.html)
- [Create a container with your own algorithms and models](https://docs.aws.amazon.com/sagemaker/latest/dg/docker-containers-create.html)
- [Adapting your own Docker container to work with SageMaker](https://docs.aws.amazon.com/sagemaker/latest/dg/docker-containers-adapt-your-own.html)
- [Build container images from your Studio notebooks](https://aws.amazon.com/blogs/machine-learning/using-the-amazon-sagemaker-studio-image-build-cli-to-build-container-images-from-your-studio-notebooks/)
- [SageMaker Environment Variables and Default Paths for Training Storage Locations](https://docs.aws.amazon.com/sagemaker/latest/dg/model-train-storage.html#model-train-storage-env-var-summary)
- [Adapting your own Docker container to work with SageMaker](https://docs.aws.amazon.com/sagemaker/latest/dg/docker-containers-adapt-your-own.html)
- [Bring your own pre-trained MXNet or TensorFlow models into Amazon SageMaker](https://aws.amazon.com/blogs/machine-learning/bring-your-own-pre-trained-mxnet-or-tensorflow-models-into-amazon-sagemaker/)
- [Use Docker containers to train and deploy models](https://docs.aws.amazon.com/sagemaker/latest/dg/docker-containers.html)
- [Dive deep into Amazon SageMaker Studio Classis Notebooks architecture](https://aws.amazon.com/blogs/machine-learning/dive-deep-into-amazon-sagemaker-studio-notebook-architecture)
- [Install External Libraries and Kernels in Amazon SageMaker Studio Classic](https://docs.aws.amazon.com/sagemaker/latest/dg/studio-notebooks-add-external.html)
- [Quick setup to Amazon SageMaker](https://docs.aws.amazon.com/sagemaker/latest/dg/onboard-quick-start.html)
- [Automate a centralized deployment of Amazon SageMaker Studio with AWS Service Catalog](https://aws.amazon.com/blogs/machine-learning/automate-a-centralized-deployment-of-amazon-sagemaker-studio-with-aws-service-catalog)
- [SageMaker Studio Permissions Required to Use Projects](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-projects-studio-updates.html)
- [Create Amazon SageMaker projects with image building CI/CD pipelines](https://aws.amazon.com/blogs/machine-learning/create-amazon-sagemaker-projects-with-image-building-ci-cd-pipelines)
- [Train, deploy, and evaluate pretrained models with SageMaker JumpStart](https://docs.aws.amazon.com/sagemaker/latest/dg/studio-jumpstart.html)
- [Collaborate with shared spaces](https://docs.aws.amazon.com/sagemaker/latest/dg/domain-space.html)
- [Persona reference](https://docs.aws.amazon.com/sagemaker/latest/dg/role-manager-personas.html)
- [ML activity reference](https://docs.aws.amazon.com/sagemaker/latest/dg/role-manager-ml-activities.html)
- [Configuring Amazon SageMaker Studio for teams and groups with complete resource isolation](https://aws.amazon.com/blogs/machine-learning/configuring-amazon-sagemaker-studio-for-teams-and-groups-with-complete-resource-isolation)
- [Best Practicies for Developing Life-Cycle Configuration Scripts for SageMaker Studio](https://github.com/aws-samples/sagemaker-studio-lifecycle-config-examples/blob/main/DEVELOPMENT.md)
- [Install External Libraries and Kernels in Amazon SageMaker Studio Classic](https://docs.aws.amazon.com/sagemaker/latest/dg/studio-notebooks-add-external.html)
- [SageMaker Studio Classic Lifecycle Configuration Samples](https://github.com/aws-samples/sagemaker-studio-lifecycle-config-examples)
- [Verify lifecycle configuration process from CloudWatch Logs](https://docs.aws.amazon.com/sagemaker/latest/dg/studio-lcc-debug.html#studio-lcc-debug-logs)
- [SageMaker Studio Classic Lifecycle Configuration Samples](https://github.com/aws-samples/sagemaker-studio-lifecycle-config-examples)
- [Customize Amazon SageMaker Studio using Lifecycle Configurations](https://aws.amazon.com/blogs/machine-learning/customize-amazon-sagemaker-studio-using-lifecycle-configurations/)
- [Using CodeWhisperer and CodeGuru extensions with SageMaker](https://docs.aws.amazon.com/sagemaker/latest/dg/sm-cw-cg.html)
- [Amazon SageMaker Studio and SageMaker Notebook Instance now come with JupyterLab 3 notebooks to boost developer productivity](https://aws.amazon.com/blogs/machine-learning/amazon-sagemaker-studio-and-sagemaker-notebook-instance-now-come-with-jupyterlab-3-notebooks-to-boost-developer-productivity/)
- [MLOps foundation roadmap for enterprises with Amazon SageMaker](https://aws.amazon.com/blogs/machine-learning/mlops-foundation-roadmap-for-enterprises-with-amazon-sagemaker/)
- [AWS Solutions Library](https://aws.amazon.com/solutions/implementations/)

## Module 3: Repeatable MLOps / Repositories

### Websites

- [Using versioning in S3 buckets](https://docs.aws.amazon.com/AmazonS3/latest/userguide/Versioning.html#Versioning)
- [Lake Formation tag-based access control](https://docs.aws.amazon.com/lake-formation/latest/dg/tag-based-access-control.html#tag-based-access-control)
- [Amazon SageMaker Feature Store](https://aws.amazon.com/sagemaker/feature-store/)
- [Create, store, and share features with Feature Store](https://docs.aws.amazon.com/sagemaker/latest/dg/feature-store.html)
- [Amazon SageMaker Data Wrangler](https://aws.amazon.com/sagemaker/data-wrangler/)
- [Batch ingestion with Amazon SageMaker Feature Store Spark](https://docs.aws.amazon.com/sagemaker/latest/dg/batch-ingestion-spark-connector-setup.html)
- [Feature Store Feature Processor SDK](https://docs.aws.amazon.com/sagemaker/latest/dg/feature-store-feature-processor-sdk.html)
- [Unlock ML insights using the Amazon SageMaker Feature Store Feature Processor](https://aws.amazon.com/blogs/machine-learning/unlock-ml-insights-using-the-amazon-sagemaker-feature-store-feature-processor)
- [Create and Manage SageMaker Pipelines](https://docs.aws.amazon.com/sagemaker/latest/dg/pipelines-build.html)
- [Register and Deploy Models with Model Registry](https://docs.aws.amazon.com/sagemaker/latest/dg/model-registry.html)
- [Register a Model Version](https://docs.aws.amazon.com/sagemaker/latest/dg/model-registry-version.html)
- [Add a Git Repository to Your Amazon SageMaker Account](https://docs.aws.amazon.com/sagemaker/latest/dg/nbi-git-resource.html)
- [Authentication and access control for AWS CodeCommit](https://docs.aws.amazon.com/codecommit/latest/userguide/auth-and-access-control.html)

## Module 4: Repeatable MLOps / Orchestration

### Websites

- [MLOps Workload Orchestrator on AWS](https://aws.amazon.com/solutions/implementations/aws-mlops-framework/)
- [Use the XGBoost algorithm with Amazon SageMaker](https://docs.aws.amazon.com/sagemaker/latest/dg/xgboost.html)
- [Orchestrate Jobs to Train and Evaluate Models with Amazon SageMaker Pipelines](https://github.com/aws/amazon-sagemaker-examples/blob/09f6fad6de75a4520f6f71d661f4b7a8139ce736/sagemaker-pipelines/tabular/abalone_build_train_deploy/sagemaker-pipelines-preprocess-train-evaluate-batch-transform.ipynb)
- [IAM: Pass an IAM role to a specific AWS service](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_policies_examples_iam-passrole-service.html)
- [IAM roles](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles.html)
- [Using the SageMaker Python SDK](https://sagemaker.readthedocs.io/en/stable/overview.html)
- [Pipeline Parameters](https://docs.aws.amazon.com/sagemaker/latest/dg/build-and-manage-parameters.html)
- [Pass Data Between Steps](https://docs.aws.amazon.com/sagemaker/latest/dg/build-and-manage-propertyfile.html)
- [Caching Pipeline Steps](https://docs.aws.amazon.com/sagemaker/latest/dg/pipelines-caching.html)
- [Selective execution of pipeline steps](https://docs.aws.amazon.com/sagemaker/latest/dg/pipelines-selective-ex.html)
- [Retry Policy for Pipeline Steps](https://docs.aws.amazon.com/sagemaker/latest/dg/pipelines-retry-policy.html)
- [Create and Manage SageMaker Pipelines](https://docs.aws.amazon.com/sagemaker/latest/dg/pipelines-build.html)
- [Amazon SageMaker pricing](https://aws.amazon.com/sagemaker/pricing/)
- [Deploy models with Amazon SageMaker Serverless Inference](https://docs.aws.amazon.com/sagemaker/latest/dg/serverless-endpoints.html)
- [Asynchronous inference](https://docs.aws.amazon.com/sagemaker/latest/dg/async-inference.html)
- [Use batch transform to run inference with Amazon SageMaker](https://docs.aws.amazon.com/sagemaker/latest/dg/batch-transform.html)
- [Amazon SageMaker pricing](https://aws.amazon.com/sagemaker/pricing/)
- [Best practices for deploying models on SageMaker Hosting Services](https://docs.aws.amazon.com/sagemaker/latest/dg/deployment-best-practices.html)
- [Amazon SageMaker Model Building Pipelines steps](https://docs.aws.amazon.com/sagemaker/latest/dg/build-and-manage-steps.html)
- [Kubernetes Orchestration](https://docs.aws.amazon.com/sagemaker/latest/dg/kubernetes-workflows.html)
- [Manage SageMaker with Step Functions](https://docs.aws.amazon.com/step-functions/latest/dg/connect-sagemaker.html)
- [Train a Machine Learning Model](https://docs.aws.amazon.com/step-functions/latest/dg/sample-train-model.html)
- [Tune a Machine Learning Model](https://docs.aws.amazon.com/step-functions/latest/dg/sample-hyper-tuning.html)
- [Preprocess data and train a machine learning model](https://docs.aws.amazon.com/step-functions/latest/dg/sample-preprocess-feature-transform.html)
- [What Is AWS Service Catalog?](https://docs.aws.amazon.com/servicecatalog/latest/dg/what-is-service-catalog.html)
- [Use SageMaker-Provided Project Templates](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-projects-templates-sm.html)
- [SageMaker Studio Permissions Required to Use Projects](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-projects-studio-updates.html)
- [AWS Managed Policies for SageMaker projects and JumpStart](https://docs.aws.amazon.com/sagemaker/latest/dg/security-iam-awsmanpol-sc.html)
- [Orchestrate XGBoost ML Pipelines with Amazon Managed Workflows for Apache Airflow](https://aws.amazon.com/blogs/machine-learning/orchestrate-xgboost-ml-pipelines-with-amazon-managed-workflows-for-apache-airflow)
- [Managing your machine learning lifecycle with MLflow and Amazon SageMaker](https://aws.amazon.com/blogs/machine-learning/managing-your-machine-learning-lifecycle-with-mlflow-and-amazon-sagemaker)
- [Securing MLflow in AWS: Fine-grained access control with AWS native services](https://aws.amazon.com/blogs/machine-learning/securing-mlflow-in-aws-fine-grained-access-control-with-aws-native-services)
- [Define customized permissions in minutes with Amazon SageMaker Role Manager](https://aws.amazon.com/blogs/machine-learning/define-customized-permissions-in-minutes-with-amazon-sagemaker-role-manager)
- [New ML Governance Tools for Amazon SageMaker – Simplify Access Control and Enhance Transparency Over Your ML Projects](https://aws.amazon.com/blogs/aws/new-ml-governance-tools-for-amazon-sagemaker-simplify-access-control-and-enhance-transparency-over-your-ml-projects/)
- [Improve governance of your machine learning models with Amazon SageMaker](https://aws.amazon.com/blogs/machine-learning/improve-governance-of-your-machine-learning-models-with-amazon-sagemaker)
- [Amazon SageMaker controls](https://docs.aws.amazon.com/securityhub/latest/userguide/sagemaker-controls.html)
- [Securing Amazon SageMaker Studio connectivity using a private VPC](https://aws.amazon.com/blogs/machine-learning/securing-amazon-sagemaker-studio-connectivity-using-a-private-vpc)
- [Connect to Amazon SageMaker resources from within a VPC](https://docs.aws.amazon.com/sagemaker/latest/dg/infrastructure-connect-to-resources.html)

## Module 5: Reliable MLOps / Scaling and testing

### Websites

- [Host models along with pre-processing logic as serial inference pipeline behind one endpoint](https://docs.aws.amazon.com/sagemaker/latest/dg/inference-pipelines.html)
- [Use a multi-container endpoint with direct invocation](https://docs.aws.amazon.com/sagemaker/latest/dg/multi-container-direct.html)
- [Configuring autoscaling inference endpoints in Amazon SageMaker](https://aws.amazon.com/blogs/machine-learning/configuring-autoscaling-inference-endpoints-in-amazon-sagemaker)
- [Step scaling policies for Application Auto Scaling](https://docs.aws.amazon.com/autoscaling/application/userguide/application-auto-scaling-step-scaling-policies.html)
- [Organizing Your AWS Environment Using Multiple Accounts](https://docs.aws.amazon.com/whitepapers/latest/organizing-your-aws-environment/organizing-your-aws-environment.html)
- [Bootstrapping multiple AWS accounts for AWS CDK using CloudFormation StackSets](https://aws.amazon.com/blogs/mt/bootstrapping-multiple-aws-accounts-for-aws-cdk-using-cloudformation-stacksets)
- [Create Custom Project Templates](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-projects-templates-custom.html)
- [Benefits of using multiple AWS accounts](https://docs.aws.amazon.com/whitepapers/latest/organizing-your-aws-environment/benefits-of-using-multiple-aws-accounts.html)
- [Operating model](https://docs.aws.amazon.com/whitepapers/latest/sagemaker-studio-admin-best-practices/operating-model.html)
- [AWS Well-Architected](https://aws.amazon.com/architecture/well-architected/)
- [Machine Learning Lens](https://docs.aws.amazon.com/wellarchitected/latest/machine-learning-lens/machine-learning-lens.html)
- [Testing phase](https://docs.aws.amazon.com/whitepapers/latest/development-and-test-on-aws/testing-phase.html)
- [Shadow tests](https://docs.aws.amazon.com/sagemaker/latest/dg/shadow-tests.html)
- Amazon Sagemaker API Reference: [InvokeEndpoint](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_runtime_InvokeEndpoint.html)
- Amazon Sagemaker API Reference: [CreateEndpoint](https://docs.aws.amazon.com/sagemaker/latest/APIReference/API_CreateEndpoint.html)
- [Production variants](https://docs.aws.amazon.com/sagemaker/latest/dg/model-ab-testing.html)
- [CloudWatch Metrics](https://docs.aws.amazon.com/sagemaker/latest/dg/model-monitor-interpreting-cloudwatch.html)
- [Update the Approval Status of a Model](https://docs.aws.amazon.com/sagemaker/latest/dg/model-registry-approve.html)
- [Perform Shadow Tests to Compare Inference Performance Between ML Model Variants](https://aws.amazon.com/blogs/aws/new-for-amazon-sagemaker-perform-shadow-tests-to-compare-inference-performance-between-ml-model-variants)
- [Monitor Amazon SageMaker with Amazon CloudWatch](https://docs.aws.amazon.com/sagemaker/latest/dg/monitoring-cloudwatch.html)
- [Getting started with CodeWhisperer](https://docs.aws.amazon.com/codewhisperer/latest/userguide/getting-started.html)

### Videos

- [Who Did This? Track Code Recommendations With Amazon CodeWhisperer](https://youtu.be/qu67bvH2Y08)

## Module 6: Reliable MLOps / Monitoring

### Websites

- [Monitor Amazon SageMaker with Amazon CloudWatch](https://docs.aws.amazon.com/sagemaker/latest/dg/monitoring-cloudwatch.html)
- [Visual explanations of core machine learning concepts](https://mlu-explain.github.io/)
- [Transform responsible AI from theory into practice](https://aws.amazon.com/machine-learning/responsible-machine-learning)
- [Amazon CloudWatch](https://aws.amazon.com/cloudwatch)
- [Amazon SageMaker Studio](https://aws.amazon.com/sagemaker/studio)
- [Model quality metrics and Amazon CloudWatch monitoring](https://docs.aws.amazon.com/sagemaker/latest/dg/model-monitor-model-quality-metrics.html)
- [SageMaker Model Monitor - visualizing monitoring results](https://sagemaker-examples.readthedocs.io/en/latest/sagemaker_model_monitor/visualization/SageMaker-Model-Monitor-Visualize.html)
- [Schedule monitoring jobs](https://docs.aws.amazon.com/sagemaker/latest/dg/model-monitor-scheduling.html)
- [Monitor Bias Drift for Models in Production](https://docs.aws.amazon.com/sagemaker/latest/dg/clarify-model-monitor-bias-drift.html)
- [Extend model lineage to include ML features using Amazon SageMaker Feature Store](https://aws.amazon.com/blogs/machine-learning/extend-model-lineage-to-include-ml-features-using-amazon-sagemaker-feature-store)
- [Amazon SageMaker pricing](https://aws.amazon.com/sagemaker/pricing)
- [Monitor Bias Drift for Models in Production](https://docs.aws.amazon.com/sagemaker/latest/dg/clarify-model-monitor-bias-drift.html)
- [Measure Pre-training Bias](https://docs.aws.amazon.com/sagemaker/latest/dg/clarify-measure-data-bias.html)
- [Measure Post-training Data and Model Bias](https://docs.aws.amazon.com/sagemaker/latest/dg/clarify-measure-post-training-bias.html)
- [Schedule model quality monitoring jobs](https://docs.aws.amazon.com/sagemaker/latest/dg/model-monitor-model-quality-schedule.html)
- [Querying Lineage Entities](https://docs.aws.amazon.com/sagemaker/latest/dg/querying-lineage-entities.html)

## Module 7: Course Wrap-up

### Websites

- [MLOps foundation roadmap for enterprises with Amazon SageMaker](https://aws.amazon.com/blogs/machine-learning/mlops-foundation-roadmap-for-enterprises-with-amazon-sagemaker)
- [Create Custom Project Templates](https://docs.aws.amazon.com/sagemaker/latest/dg/sagemaker-projects-templates-custom.html)
- [Custom Project Templates in SageMaker](https://github.com/aws-samples/sagemaker-custom-project-templates)
- [Amazon SageMaker for MLOps](https://aws.amazon.com/sagemaker/mlops)
- [MLOps Workload Orchestrator on AWS](https://aws.amazon.com/solutions/implementations/mlops-workload-orchestrator)
- [AWS Certification exam preparation](https://aws.amazon.com/certification/certification-prep/)
- [AWS Certification](https://aws.amazon.com/certification/)
- [AWS Recertification](https://aws.amazon.com/certification/recertification/)
- [All AWS Certification exams](https://aws.amazon.com/certification/exams/)
- [Schedule an Exam](https://aws.amazon.com/certification/certification-prep/testing/)
- AWS Skill Builder [subscription](https://aws.amazon.com/training/digital)
- [AWS Training and Certification](https://aws.amazon.com/training)
- [AWS Online Tech Talks](https://aws.amazon.com/events/online-tech-talks/on-demand/)
- [AWS Ramp-Up Guides](https://aws.amazon.com/training/ramp-up-guides/)

### Courses

- [AWS Skill Builder](https://explore.skillbuilder.aws/learn)
- [AWS Builder Labs](https://aws.amazon.com/training/digital/aws-builder-labs/)
- [AWS Workshops](https://workshops.aws/)