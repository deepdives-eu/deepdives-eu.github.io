# Introduction to Data Analytics on Google Cloud links

## Module 00: Course Introduction

Full name: Introduction to Data Analytics on Google Cloud

Product code: T-IDAGC-B

These links are based on the current course version.

Online course options:
- Introduction to Data Analytics on Google Cloud - [Google Cloud Skills Boost](https://www.cloudskillsboost.google/course_templates/578)
- Introduction to Data Analytics on Google Cloud - [Coursera](https://www.coursera.org/learn/introduction-to-data-analytics-on-google-cloud)

## Module 01: Understanding the Data Analytics Lifecycle on Google Cloud

### Websites
- What is data analytics? <https://cloud.google.com/learn/what-is-data-analytics>
- Cloud Storage documentation <https://cloud.google.com/storage/docs>
- Cloud Storage classes <https://cloud.google.com/storage/docs/storage-classes>
- Pub/Sub overview <https://cloud.google.com/pubsub/docs/overview>
- Dataproc documentation <https://cloud.google.com/dataproc/docs>
- Dataflow documentation <https://cloud.google.com/dataflow/docs>
- Cloud Data Fusion documentation <https://cloud.google.com/data-fusion/docs>
- Cloud SQL documentation <https://cloud.google.com/sql/docs>
- AlloyDB for PostgreSQL <https://cloud.google.com/alloydb/docs>
- Cloud Spanner documentation <https://cloud.google.com/spanner/docs>
- Firestore documentation <https://cloud.google.com/firestore/docs>
- Bigtable documentation <https://cloud.google.com/bigtable/docs>
- BigQuery overview <https://cloud.google.com/bigquery/docs/introduction>
- Looker documentation <https://cloud.google.com/looker/docs>
- Vertex AI documentation <https://cloud.google.com/vertex-ai/docs>
- Database options on Google Cloud <https://cloud.google.com/products/databases>

### Videos
- What is Cloud Storage? <https://www.youtube.com/watch?v=VDBhfixAbAs>
- What is Pub/Sub? <https://www.youtube.com/watch?v=cvu53CnZmGI>
- What is Dataflow? <https://www.youtube.com/watch?v=KalJ0VuEM7s>
- What is Dataproc? <https://www.youtube.com/watch?v=3W5RiOfMsV4>
- What is Looker? <https://www.youtube.com/watch?v=bVDMo4XnfaE>
- What is Vertex AI? <https://www.youtube.com/watch?v=gT4qqHMiEpA>
- What is Cloud SQL? <https://www.youtube.com/watch?v=DK-BknkGRRo>
- What is Cloud Spanner? <https://www.youtube.com/watch?v=bUSU1e9QQmA>
- What is Bigtable? <https://www.youtube.com/watch?v=KDey3FUsgoI>

## Module 02: Exploring Data and Extracting Insights Using BigQuery

### Websites
- BigQuery documentation <https://cloud.google.com/bigquery/docs>
- Introduction to BigQuery <https://cloud.google.com/bigquery/docs/introduction>
- Loading data into BigQuery <https://cloud.google.com/bigquery/docs/loading-data>
- Querying data in BigQuery <https://cloud.google.com/bigquery/docs/running-queries>
- BigQuery SQL reference <https://cloud.google.com/bigquery/docs/reference/standard-sql/query-syntax>
- DDL statements in BigQuery <https://cloud.google.com/bigquery/docs/reference/standard-sql/data-definition-language>
- DML statements in BigQuery <https://cloud.google.com/bigquery/docs/reference/standard-sql/dml-syntax>
- Views in BigQuery <https://cloud.google.com/bigquery/docs/views>
- Materialized views <https://cloud.google.com/bigquery/docs/materialized-views-intro>
- User-defined functions (UDFs) <https://cloud.google.com/bigquery/docs/reference/standard-sql/user-defined-functions>
- BigQuery ML overview <https://cloud.google.com/bigquery/docs/bqml-introduction>
- Scheduled queries <https://cloud.google.com/bigquery/docs/scheduling-queries>
- BigLake overview <https://cloud.google.com/biglake/docs/introduction>
- External tables <https://cloud.google.com/bigquery/docs/external-tables>
- Querying Google Sheets data <https://cloud.google.com/bigquery/docs/external-data-drive>
- Federated queries <https://cloud.google.com/bigquery/docs/federated-queries-intro>
- BigQuery GIS (Geospatial analytics) <https://cloud.google.com/bigquery/docs/geospatial-intro>
- Nested and repeated fields <https://cloud.google.com/bigquery/docs/nested-repeated>
- BigQuery DataFrames (bigframes) <https://cloud.google.com/bigquery/docs/bigquery-dataframes-introduction>
- Gemini in BigQuery <https://cloud.google.com/gemini/docs/bigquery/overview>
- BigQuery data canvas <https://cloud.google.com/bigquery/docs/data-canvas>
- Python notebooks in BigQuery <https://cloud.google.com/bigquery/docs/notebooks-introduction>
- bq command-line tool <https://cloud.google.com/bigquery/docs/bq-command-line-tool>

### Videos
- Getting started with BigQuery <https://www.youtube.com/watch?v=d3MDxC_iuaw>
- BigQuery explained: storage overview <https://www.youtube.com/watch?v=a0jeP5kuzIE>
- BigQuery explained: querying data <https://www.youtube.com/watch?v=GrDWGfCUB08>
- Introduction to BigQuery ML <https://www.youtube.com/watch?v=KORMRnbzUr4>
- What is BigLake? <https://www.youtube.com/watch?v=GSyc99P5PQQ>
- BigQuery GIS tutorial <https://www.youtube.com/watch?v=u6TIi56uyeM>
- BigQuery DataFrames <https://www.youtube.com/watch?v=SdclYLvHxYo>
- Gemini in BigQuery <https://www.youtube.com/watch?v=W2BhRMKBgis>

## Module 03: Making Data-Driven Decisions Using Looker

### Websites
- Looker documentation <https://cloud.google.com/looker/docs>
- Looker Explores <https://cloud.google.com/looker/docs/exploring-data>
- Dimensions and measures in Looker <https://cloud.google.com/looker/docs/reference/param-explore-measure>
- Looker visualizations <https://cloud.google.com/looker/docs/best-practices/visualizations>
- Looker dashboards <https://cloud.google.com/looker/docs/building-dashboards>
- Looker Studio documentation <https://support.google.com/looker-studio>
- Looker Studio overview <https://cloud.google.com/looker-studio>
- Connecting Looker Studio to BigQuery <https://support.google.com/looker-studio/answer/6370296>
- Sharing Looker reports <https://cloud.google.com/looker/docs/sharing-looks>
- LookML overview <https://cloud.google.com/looker/docs/what-is-lookml>

### Videos
- What is Looker? <https://www.youtube.com/watch?v=bVDMo4XnfaE>
- Getting started with Looker <https://www.youtube.com/watch?v=KJhVhs0gD_Y>
- Introduction to Looker Studio <https://www.youtube.com/watch?v=6FMQonqjsGs>
- Building dashboards with Looker Studio <https://www.youtube.com/watch?v=a5Wn5v8JnvE>
- Looker: Explores and Dimensions <https://www.youtube.com/watch?v=Z8Y1HGoUkbw>

## Module 04: Course Summary

### Websites
- Google Cloud Data Analytics Learning Path <https://cloud.google.com/learn/training/data-engineering-and-analytics>
- Google Cloud Certification - Professional Data Engineer <https://cloud.google.com/learn/certification/data-engineer>
- Google Cloud Architecture Center <https://cloud.google.com/architecture>
- Google Cloud blog - Data Analytics <https://cloud.google.com/blog/products/data-analytics>
- BigQuery public datasets <https://cloud.google.com/bigquery/public-data>
